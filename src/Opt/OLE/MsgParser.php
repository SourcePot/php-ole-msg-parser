...
class MsgParser {
    ...
    /** @var array<int, array{0:string,1:string}> Mapping of property tags originally from https://github.com/mvz/email-outlook-message-perl/blob/master/mapitags.h */
    ...
    protected function parseMessageStream(string $data, bool $isTopLevel, array $container, string $basePath):stdClass
    {
        ...
        if (isset($props['MESSAGE_DELIVERY_TIME'])) {
            ...
            unset($props['MESSAGE_DELIVERY_TIME']);
        }
        if (isset($msg->headers['BODY'])){
            unset($msg->headers['BODY']);
        }
        if (isset($msg->headers['RTF_COMPRESSED'])){
            unset($msg->headers['RTF_COMPRESSED']);
        }
        $msg->body = '';
        if (isset($props['BODY'])) {
            $msg->body = $props['BODY'];
    ...
    protected function parseProperties(string $data, bool $isTopLevel, array $container, string $basePath):array
    {
        ...
        while ($offset + 16 <= strlen($data)) {
            ...
            if (!isset(self::PROPERTY_TAGS[$ptag])) {
                $offset += 16;
                continue;
            }
    ...
    protected function getEncodings(array $props, string $tagName):array
    {
        ...
        $propertiesEncoding = isset($props['PR_MESSAGE_CODEPAGE']) && isset(self::CODE_PAGES[$props['PR_MESSAGE_CODEPAGE']])
            ? self::CODE_PAGES[$props['PR_MESSAGE_CODEPAGE']] : NULL;
        if ($tagName === "BODY") {
